...
data = {
    'ObjPort': {'rom': {'Baseline': [250], 'Dev': [130], 'Min': [80]}, 'ram': {'Baseline': [88], 'Dev': [88], 'Min': [4]}},
...
def magic_plot(components:list[str], memory_type:str, bar_width=0.2, spacing=0.02):
    ...
    plt.bar(x_axis - bar_width - spacing, baseline_data, bar_width, label = "Baseline", color=colors[0])
    plt.bar(x_axis, dev_data, bar_width, label = 'Dev', color=colors[1])
    plt.bar(x_axis + bar_width + spacing, min_data, bar_width, label = 'Min', color=colors[2])
    ...
    for x_off, cur_data in [(- bar_width - spacing, baseline_data), (0, dev_data), (bar_width + spacing, min_data)]:
        for i, val in enumerate(cur_data):
            plt.text(x_axis[i] + x_off + 0.01, 2**(math.log2(val) - 0.3), '{:,}'.format(val).replace(',', ' '), rotation=90, ha='center', va='top')

    plt.yscale('log') 
    plt.xticks(x_axis, components, fontsize=11, fontweight='semibold')
    title_mem_type = 'RAM' if memory_type == "ram" else "Flash"
    plt.title(f'{title_mem_type} Usage of Standard Components', fontweight='bold' ,fontsize=15, pad=18)
    plt.ylabel(f"{title_mem_type} usage", fontsize=11, fontweight='semibold')
...
magic_plot(components=components, memory_type='rom', bar_width=0.2)